...
class QhDataBase: public QObject
{
    Q_OBJECT
...
public:
    ...
    /// @param insertId：返回插入数据的ID，目前只对高斯数据库（插入语句示例：INSERT INTO user (name) VALUES ('u3') RETURNING id;）
    /// @param datas：绑定参数
    /// @return 错误返回值小于0
    int execAffectedRows(const QString& sql,
                         qint64 *lastInsertId = nullptr,
