...
QString QhProgressBar::secondaryText() const
{
    return d->secondaryText;
}

QFont QhProgressBar::secondaryTextFont() const
{
    return d->secondaryTextFont;
}

QPen QhProgressBar::secondaryTextPen() const
{
    return d->secondaryTextPen;
}

QString QhProgressBar::fixedText() const
{
    return d->fixedText;
}
...
void QhProgressBar::setSecondaryText(const QString &text)
{
    d->secondaryText = text;
    this->update();
}

void QhProgressBar::setSecondaryTextFont(const QFont &font)
{
    d->secondaryTextFont = font;
    this->update();
}

void QhProgressBar::setSecondaryTextPen(const QPen &pen)
{
    d->secondaryTextPen = pen;
    this->update();
}

void QhProgressBar::setFixedText(const QString &text)
{
    d->fixedText = text;
    this->update();
}
...
void QhProgressBarPrivate::drawText(QPainter &p)
{
    ...
    case QhProgressBar::AlignCenter: {
        if (this->secondaryText.isEmpty()) {
            p.drawText(pBar->rect(), Qt::AlignCenter, fixedText.isEmpty() ? text : fixedText);
        } else {
            int space = 12;
            auto rect = pBar->rect();
            QRect rectTop = QRect(rect.x(), rect.y(), rect.width(), rect.height() / 2 - space / 2);
            QRect rectBottom = QRect(rect.x(), rect.y() + rect.height() / 2 + space / 2,
                rect.width(), rect.height() / 2 - space / 2);

            p.drawText(rectTop, Qt::AlignHCenter | Qt::AlignBottom, fixedText.isEmpty() ? text : fixedText);
...
//            p.setFont(font);
            p.setFont(secondaryTextFont);
            p.setPen(secondaryTextPen);
            p.drawText(rectBottom, Qt::AlignHCenter | Qt::AlignTop, this->secondaryText);
        }
        break;
    }
